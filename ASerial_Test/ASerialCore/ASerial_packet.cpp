...
int ASerialPacket::MakePacketData(uint8_t* to_device_data, int data_num, uint8_t command, uint8_t* data_packet_out)
{
    ...
    // ★★★ ここから修正箇所1 ★★★
    uint8_t check_data[2] = { 0 };  // uint8_t に変更

    check_data[0] = (uint8_t)((check_sum & 0xFF00) >> 8);  // 上位バイト抽出 -> インデックスを 0 に
    check_data[1] = (uint8_t)(check_sum & 0x00FF);        // 下位バイト抽出 -> インデックスを 1 に

    CheckDataNeedAddFlag(check_data[0], data_packet_out, &index); // インデックスを 0 に
    ++index;
    CheckDataNeedAddFlag(check_data[1], data_packet_out, &index); // インデックスを 1 に
    // ★★★ ここまで修正箇所1 ★★★
...
void ASerialPacket::ResetDataArray(void)
{
    ...
    if (GetMode() == MODE_DEVICE) {
        // m_from_device_data_buf は存在しないため、m_from_controller_data_buf を使用
    ...
    // sizeof(array) はポインタのサイズを返すため、固定長を使用
    for (int i = 0; i < _ASERIAL_MAX_DATA_NUM; ++i) {
        array[i] = (uint8_t)0;
    }
}
